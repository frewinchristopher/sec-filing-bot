                    ...
                    if company_name[last25] not in stocks_sent:
                        ...
                        if t in str(today):
                            try:
                               ...
                               headers = {'Ocp-Apim-Subscription-Key': os.environ.get('LAST10K_API_PRIMARY_KEY'),}
...
def email(symbol, address, current_price, company_name, message):
    ...
    smtpObj.ehlo()
    smtpObj.starttls()
    smtpObj.login(os.environ.get('GMAIL_ADDRESS'), os.environ.get('GMAIL_PASSWORD'))
    print(smtpObj.sendmail(os.environ.get('GMAIL_ADDRESS'),\
